...
<script
	src="${pageContext.request.contextPath}/js/pc/schwartz/common.js"></script>
<script
	src="${pageContext.request.contextPath}/js/pc/schwartz/basket_multiply.js"></script>
<script
...
// 				}
	...
	function showFirstTimeDiv() {
		...
		//alert(screenH+"====="+scrollH+"---"+divHeight);
	...
	function showOfferInfo(obj) {
			...
			if (isNotEmpty(v)) {
				...
				if (isNotEmpty(v)) {
					...
					if (v == 1)
						desp += "any 2 for 2";
					if (v == 2)
						desp += "Save 99p was £1.99 now £1.00";
					if (v == 3)
						desp += "Save 49p was £1.99 now £1.50";
					if (v == 4)
						desp += "Any 2 for £1.50";
					if (v == 5)
						desp += "Better Than Half Price Was £2.49 Now £1.00";
					if (v == 6)
						desp += "Better Than Half Price Was £2.75 Now £1.00";
					if (v == 7)
						desp += "Buy any 2 for £1.20";
					if (v == 8)
						desp += "Any 2 for £3.00";
					if (v == 10)
						desp += "Save 39p was £2.39 now £2.00";
					$("#mulitiple_product_offer").find("li:eq(0)").html(desp);
				} else
					$("#mulitiple_product_offer").find("li:eq(0)").html(
							"OFFER APPLIED<br/> No offer");
			} else
				$("#mulitiple_product_offer").find("li:eq(0)").html(
						"OFFER APPLIED<br/> No offer");
		} else {
		...
		$("#mulitiple_product_offer").css("position", "absolute");
		$("#mulitiple_product_offer").offset({
			"left" : left - 75.5,
			"top" : top - 45
		}).css("display", "block");
		// }
...
// 		});
	...
	function hideOfferInfo(obj) {
		$("#mulitiple_product_offer").removeAttr("style").hide();
	}
</script>
<!-- Piwik --> 
			...
			<div class="openstore_retailer">
							...
							<c:choose>
								<c:when test="${retailer.name eq defaultRetailer}">
									<li class="sch_retailer_active"
										style="width: 98px; padding-top: 2px;"><img
				...
				<a
					href="${pageContext.request.contextPath}/${brandModel['brandName']}/${defaultRetailer}/${defaultCategory}/help"><div
						class="sch_retailer_help">HELP</div> </a>
			</div>
			<div class="clear"></div>
			...
			<div class="openstore_block">
				<div class="sch_block_left">
					<div class="block_left_con">
						...
						<div class="sch_addpro_top">
							${defaultCategory} <span
								style="font-weight: normal; color: #AAAAAA;"> |
								${defaultCategorySecondName}</span>
						</div>

						<div class="sch_addpro_note" style="display: none;">
							<span> BUY ONE, GET ONE FREE</span><br /> on all Slow Cookers
							Recipe Mixes
						</div>
						...
						<div class="sch_pro_add">
											...
											<ul
												...
												<li class="sch_pro_info"><a
													href="${pageContext.request.contextPath}/${brandModel['brandName']}/${defaultRetailer}/${defaultCategory}/product/${product.productId}"><img
														src="${pageContext.request.contextPath}/images/pc/schwartz//info_btn.png"
														height="14" name="" border="0"> </a></li>
												<li class="sch_pro_title" style="overflow:hidden;max-height:22px;" title="${product.productName}"><a
													href="${pageContext.request.contextPath}/${brandModel['brandName']}/${defaultRetailer}/${defaultCategory}/product/${product.productId}">${product.productName}</a>
												</li>
												<li class="sch_pro_price">£${product.nowPrice}</li>
												<li class="sch_pro_qty">
							...
							<c:if test="${fn:length(productList) <= 0}">
								<span class="sch_pro_text">Not currently stocked at your
									selected retailer. Please try another retailer</span>
							</c:if>
				...
				<div class="sch_block_right">
							...
							<div class="sch_login_acc">
								<ul>
									<li><input class="sch_input_acc" name=""
										placeholder="${fn:toUpperCase(defaultRetailer)} username"
										id="userName"  onchange="saveUserName('userName')"/></li>
									<li><input class="sch_input_acc" name="password"
										placeholder="${fn:toUpperCase(defaultRetailer)} password"
										type="password" id="password" /></li>
					...
					<div class="basket_panel_con3" style="display: none;">
						<ul class="basket_con3_ula"
							...
							<li class="basket_con3_li01">SORRY, THE FOLLOWING PRODUCTS
						...
						<ul class="basket_con3_ula" style="font-weight: bold;">
					...
					<div class="basket_panel_con4" style="display: none">
						<ul class="basket_con3_ula"
							...
							<li class="basket_con3_li01"
								style="font-family: Arial; font-weight: bold;">SORRY YOUR
								USERNAME AND PASSWORD COMBINATION IS NOT RECOGNISED BY
								${defaultRetailerUpperCase}, PLEASE TRY AGAIN.</li>
						</ul>
	...
	<div class="sch_retailer_logo2">
		...
		<c:forEach items="${retailerList}" var="retailer">
							...
							<c:choose>
								<c:when test="${retailer.name eq defaultRetailer}">
									<li class=""
										style="width: 86px; padding-top: 2px;" onclick="closeMsgDiv2();"><a href="${pageContext.request.contextPath}/${brandModel['brandName']}/${retailer.name}/${defaultCategory}/product"><img
										src="${pageContext.request.contextPath}/images/logo_${fn:toLowerCase(retailer['name'])}01.png"
										height="17" name="" border="0"></li>
								</c:when>
								<c:otherwise>
