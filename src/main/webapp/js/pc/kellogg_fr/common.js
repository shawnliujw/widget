...
function showMsgDiv(msg,flag,title,type,left,top,itemArray) {
	...
	if(type == 'errorMsg') {
		div = '<div id="floatMsgDiv" class="basket_panel_con" style="z-index:100000;position: absolute;width: 300px;list-style: none; text-shadow: none;">'+		
		'<li class="basket_con3_li01" style="font-size: 11px; color: #BF314F;height:50px; padding: 30px 18px 10px 18px;text-align: center;"  id="floatMsgDivContent"></li>'+
		'<li onclick="closeMsgDiv();" style="padding: 8px 0px 20px 0px;text-align: center;"><input style="width:170px;" class="input_btn02" type="button" value="BACK" name="" /></li></div>';
	} 
...
$().ready(function(){
	...
	if(nav.indexOf("Microsoft")!=-1 || nav.indexOf("Netscape") != -1){
		var doc = document, inputs = doc.getElementsByTagName('input'),
		 ...
		 placeholder = function(input) {
			...
			input.onfocus = function() { 
				...
				input.setAttribute('style',"color:#000000;");
...
function goToRetailer(temp){
	...
	if(obj.toLowerCase().indexOf("tesco")!=-1){
		...
		window.open("http://www.sainsburys.co.uk/groceries/index.jsp?bmUID=1353998280627");
	}else if(obj.toLowerCase().indexOf("waitrose")!=-1){
		window.open("http://www.waitrose.com/");
	} else if(obj.toLowerCase().indexOf("auchan")!=-1){
		window.open("http://www.auchandirect.fr");
	} else if(obj.toLowerCase().indexOf("monoprix")!=-1){
		window.open("http://courses.monoprix.fr");
	} else if(obj.toLowerCase().indexOf("ooshop")!=-1){
		window.open("http://www.carrefour.fr");
	} 
}
