...
/**
 * RemoteStorage object , let user can store data into remote server and retrive them
 * 
 * @author Shawn
 */
window.remoteStorage=(function RemoteStorage() {
	var remoteStorage={};
	var remotePath = encodeUrl("/storage");
	var publicMembers = {
		retrieve: function() {
			$.ajax({
				url: remotePath,
				...
				success: function(response) {
					if(response != '') {
						remoteStorage=response;
					} 
				}
			});
		},
		//use url as parameter if you want to take user to other page
		save: function(url) {
			// Update the timestamp
			remoteStorage.timestamp=new Date().getTime();
			// Save the data
			$.ajax({
				url: remotePath,
				type: "post",
				data: {"jsonData":JSON.stringify(remoteStorage)},
				success: function(response) {
//					console.log("Saved data to server at ",remoteStorage.timestamp,remoteStorage);
					if(undefined != url) {
						url=encodeUrl(url);
		...
		getItem: function(name) {
			return remoteStorage[name];
		},

		setItem: function(name, value) {
			remoteStorage[name]=value;
		},
...
function saveModel(modelName,model) {
	...
	if(isNotEmpty(modelName)) {
		...
		if(typeof(model) == 'object') {
			...
			remoteStorage.setItem(modelName,model)
		}
	} else {
		window.console.log("User has disabled storing local content");
		alert("Sorry, you must allow this site to store local data to continue. You can change this policy in your browser's security settings.");
...
function encodeUrl(url) {
	try{
		...
		if (index != -1) {
			...
			url = url + ";jsessionid=" + sessionId + "?dateTime=" + (new Date()).getTime();
		}
		...
		alert("error when call method encodeUrl()");
		//window.console.log("User has disabled storing local content");
		//alert("Sorry, you must allow this site to store local data to continue. You can change this policy in your browser's security settings.");
	}
}
...
function showMsgDiv(msg,flag,title,type,left,top,itemArray) {
	...
	if(type == 'errorMsg') {
		div = '<div id="floatMsgDiv" class="basket_panel_con" style="z-index:900000;position: absolute;width: 350px; height:100px; list-style: none; text-shadow: none;">'+		
		'<li class="basket_con3_li01" style="font-size: 11px; color: #000000;height:20px; padding: 20px 18px 10px 18px;text-align: center;"  id="floatMsgDivContent"></li>'+
		'<li onclick="closeMsgDiv();" style="padding: 8px 0px 20px 0px;text-align: center; cursor: pointer;"><img src="'+path+'/images/pc/johnsonbaby/back_btn.png"  name="" border="0" /> </li></div>';
	} 
...
function showMaskLayerWithoutLoading(){
	var div = '<div id="floatMaskLayerDiv" style="position: absolute; left: 0px; width: 100%; height: 100%;text-align:center;filter: Alpha(Opacity =         30); opacity: 0.3; background-color: #000000;z-index:100000; ">';
	div+='</div>';
	$("body").append($(div));
	$("#floatMaskLayerDiv").offset({
				"left":0,
	...
	if (!isPlaceholder()) {//不支持placeholder 用jquery来完成  
	    $(document).ready(function() {  
	        if(!isPlaceholder()){  
	            ...
	            pwdField.after('<input id="pwdPlaceholder" type="text" class="new_input" value="'+pwdVal+'" />');  
...
function goToRetailer(temp){
	...
	if(obj.toLowerCase().indexOf("tesco")!=-1){
		rtl="tesco";
		window.open("http://www.tesco.com/groceries/Basket/");
	}else if(obj.toLowerCase().indexOf("asda")!=-1){
		rtl="asda";
		window.open("http://groceries.asda.com/asda-estore/checkout/regularviewtrolleycontainer.jsp?requestedURL=/asda-estore/home/basket-greyedout-hero.jsp");
	}else if(obj.toLowerCase().indexOf("sainsbury")!=-1){
		rtl="sainsbury";
		window.open("http://www.sainsburys.co.uk/groceries/trolley/trolley.jsp");
	}else if(obj.toLowerCase().indexOf("boot")!=-1){
		rtl="boots";
		window.open("http://www.boots.com/webapp/wcs/stores/servlet/OrderItemDisplay?URL=OrderItemDisplay%3ForderType%3Dregular&storeId=10052&orderType=regular&updatePrices=1&calculationUsageId=-1&langId=-1&catalogId=11051");
	}else if(obj.toLowerCase().indexOf("pharmacy")!=-1){
		rtl="pharmacy2u";
		window.open("http://www.pharmacy2u.co.uk/ssl/checkout/basket.aspx");
	}else if(obj.toLowerCase().indexOf("amazon")!=-1){
		rtl="amazon";
		window.open("http://www.amazon.co.uk/gp/cart/view.html/ref=gno_cart");
	}
	_gaq.push(['_trackevent', 'ext link', 'click', rtl , , 'true']);
	_gaq.push(['b._trackevent', 'ext link', 'click', rtl , , 'true']);

}

function closeCreateAccountMsgDiv(){
	$("#floatCreateAccountMsgDiv").remove();
}

function showCreateAccountMsgDiv(msg,flag,title,type,left,top,itemArray) {
//	closeMsgDiv();
	//showMsgDiv(pageId);
	//showMsgDiv(left+"---"+top);
...
//	if(flag) showMaskLayerWithoutLoading();
	...
	if(type == 'errorMsg') {
		div = '<div id="floatCreateAccountMsgDiv" class="errors_con" style="z-index:900000;position: absolute;width: 350px; height:100px; list-style: none; text-shadow: none;">'+		
		'<li class="basket_con3_li01" style="font-size: 11px; color: #fff;height:20px; padding: 20px 18px 10px 18px;text-align: center;"  id="floatCreateAccountMsgDivContent"></li>'+
		'<li onclick="closeCreateAccountMsgDiv();" style="padding: 8px 0px 20px 0px;text-align: center; cursor: pointer;"><div class="store_btn" style="width: 120px; margin: 0 auto;" >CONTINUE</div> </li></div>';
	} 
	...
	if(isNotEmpty(title)) {
		$("#floatCreateAccountMsgDivTitle").text(title);
	}
	
	if(isNotEmpty(msg)) {
		
		$("#floatCreateAccountMsgDivContent").html(msg);
	}
	var divLeft = 0;
	var divTop = 0;
	var divHeight = $("#floatCreateAccountMsgDiv").height();
	var divWidth = $(div).width();
	...
	//showMsgDiv(divLeft+"---"+divTop);
	$("#floatCreateAccountMsgDiv").offset({
				"left":divLeft,
				"top":divTop
			}).css("display","");
} 
