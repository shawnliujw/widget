...
{ 
	...
	//showMsgDiv(1.1*Math.pow(10, 2)/Math.pow(10, 2));
...
function showMsgDiv(msg,flag,title,type,left,top,itemArray) {
	...
	if(type == 'errorMsg') {
		div = '<div id="floatMsgDiv" class="basket_panel_con" style="font-family:Arial; z-index:100000;position: absolute;width: 300px;list-style: none; text-shadow: none;">'+		
		'<li style="font-size: 11px; height:50px; font-weight:bold; color: #C72B31; padding: 30px 18px 10px 18px;text-align: center;"  id="floatMsgDivContent"></li>'+
...
function goToRetailer(temp){
	...
	if(obj.toLowerCase().indexOf("tesco")!=-1){
		window.open("http://www.tesco.com/groceries");
		...
		window.open("http://groceries.asda.com/asda-estore/home/homecontainer.jsp");
	}else if(obj.toLowerCase().indexOf("sainsbury")!=-1){
		window.open("http://www.sainsburys.co.uk/groceries/index.jsp?bmUID=1354209401843");
