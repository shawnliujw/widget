...
function initBasketPageWithProduct(){
	...
	if(isNotEmpty(current)){
		...
		for ( var v = current.length; v--; v >= 0) {
			...
			var pb='<div style="margin-bottom:10px;">';
			pb+='<div class="bsk_proname">';
			pb+='<ul> <li >'+item.title+'</li> </ul>';
			pb+='</div> <div class="bsk_proprice"> <ul>';
			pb+='<li style="float:left; font-weight: bold; margin-left:17px;">x'+item.quantity+'</li>';
			pb+='<li style="margin-top:-15px; margin-right:14px;"><input value="'+item.productId+'" type="hidden"/><img src="'+path+'/images/mobile/kellogg_fr/icon_del.png" onclick="removeItemById(this);" width="38px;" height="34px" name="" border="0"></li>';
			pb+='<li style="width:45px; padding-left:7px; ">€'+formatFloat(item.price * item.quantity,2)+'</li>';
